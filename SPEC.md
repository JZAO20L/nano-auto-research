...
- **轮次上限**：5 轮
- **每轮搜索上限**：最多 5 组关键词，每组取 top 20 篇（`max_results=20`），单轮深读 ≤ 10 篇
- **每轮搜索策略**：
...
2. 搭建统一评测基建（数据加载 + 评估指标 + 评估脚本，规范见 s2-flow 2.2）
...
7. 执行实验、收集结果（命名规范和失败处理见 s2-flow 2.4）
