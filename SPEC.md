...
**搜索不限定固定轮次**，而是设定总轮次上限 + 完成度评估逻辑：

- **总轮次上限**：默认 10 轮（可由用户在 topic 输入时覆盖）
- **每轮搜索策略由 agent 自主决定**，参考方向：
  - 早期轮次：宽泛 topic 词 → 建立领域地图
  - 中期轮次：从高相关论文中提取方法名/术语精搜 → 定位技术路线
  - 后期轮次：cite-chasing（从高相关论文的 related work 追溯）→ 补漏
- **完成度评估逻辑**（每轮结束后 agent 自评）：
  1. Gap 是否已明确且稳定？（连续两轮反思未修改 gap 核心描述 → 稳定）
  2. 最近两轮是否仍有新的高相关论文？（>90% 已见过 → 饱和）
  3. 是否已覆盖主要技术路线？（领域内 2-3 条主流路线均有代表论文）
  4. Positioning 是否可写？（能明确说出与最近 2-3 篇的区别）
- **阶段结束条件**：完成度评估 4 项中至少 3 项满足，或达到总轮次上限
...
## Status
- 当前阶段：searching / analyzing / idea_generation / awaiting_user_decision
- 已完成搜索轮次：N
- 已读论文数：N
- 完成度自评：gap稳定=Y/N, 搜索饱和=Y/N, 路线覆盖=Y/N, positioning可写=Y/N
```
...
├── model_config.yaml                ← 模型调用配置（URL/Key/模型名，用户填写）
...
| `model_config.yaml` | 用户 | 模型 URL/API Key/名称 | S2 前填写 |
